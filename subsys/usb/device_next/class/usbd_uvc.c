...
static uint8_t uvc_get_bulk_in(const struct device *dev)
{
	...
	case USBD_SPEED_SS:
		return cfg->desc->if1_ep_ss.bEndpointAddress;
	default:
		CODE_UNREACHABLE;
	}
}
...
static void *uvc_get_desc(struct usbd_class_data *const c_data, const enum usbd_speed speed)
{
	...
	case USBD_SPEED_SS:
		cfg->desc->if1_hdr.bEndpointAddress = cfg->desc->if1_ep_ss.bEndpointAddress;
		return (void *)cfg->ss_desc;
	default:
		CODE_UNREACHABLE;
	}
}
...
static union uvc_fmt_desc *uvc_new_fmt_desc(const struct device *dev)
{
	...
	ret = uvc_add_desc(dev, desc, true, true, true);
	if (ret != 0) {
		return NULL;
	}
...
static int uvc_init(struct usbd_class_data *const c_data)
{
	...
	uvc_add_desc(dev, &cfg->desc->if1_ep_ss, false, false, true);
	uvc_add_desc(dev, &cfg->desc->if1_ep_ss_co, false, false, true);
