...
#define DT_DRV_COMPAT lattice_csi_rx
...
struct lscc_csi_rx_reg {
	uint32_t addr;
	const char *name;
	const char *description;
};

static int lscc_csi_rx_init(const struct device *dev)
{
...
static int lscc_csi_rx_set_format(const struct device *dev, enum video_endpoint_id ep,
...
static int lscc_csi_rx_get_format(const struct device *dev, enum video_endpoint_id ep,
			      struct video_format *fmt)
{
	const struct lscc_csi_rx_config *cfg = dev->config;
...
static int lscc_csi_rx_get_caps(const struct device *dev, enum video_endpoint_id ep,
...
static int lscc_csi_rx_set_frmival(const struct device *dev, enum video_endpoint_id ep,
...
static int lscc_csi_rx_get_frmival(const struct device *dev, enum video_endpoint_id ep,
			       struct video_frmival *frmival)
{
	const struct lscc_csi_rx_config *cfg = dev->config;
...
static int lscc_csi_rx_enum_frmival(const struct device *dev, enum video_endpoint_id ep,
...
static int lscc_csi_rx_set_stream(const struct device *dev, bool on)
{
	const struct lscc_csi_rx_config *cfg = dev->config;
...
static int lscc_csi_rx_set_ctrl(const struct device *dev, unsigned int cid, void *value)
{
	const struct lscc_csi_rx_config *cfg = dev->config;
...
static int lscc_csi_rx_get_ctrl(const struct device *dev, unsigned int cid, void *value)
{
	const struct lscc_csi_rx_config *cfg = dev->config;
...
static const DEVICE_API(video, lscc_csi_rx_driver_api) = {
	.set_format = lscc_csi_rx_set_format,
	.get_format = lscc_csi_rx_get_format,
	.get_caps = lscc_csi_rx_get_caps,
	.set_frmival = lscc_csi_rx_set_frmival,
	.get_frmival = lscc_csi_rx_get_frmival,
	.enum_frmival = lscc_csi_rx_enum_frmival,
	.set_stream = lscc_csi_rx_set_stream,
	.set_ctrl = lscc_csi_rx_set_ctrl,
	.get_ctrl = lscc_csi_rx_get_ctrl,
};
...
#define LSCC_CSI_RX_DEVICE_DEFINE(inst)                                                            \
	const struct lscc_csi_rx_config lscc_csi_rx_cfg_##inst = {                                 \
		...
		.base = DT_INST_REG_ADDR(inst),                                                    \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, lscc_csi_rx_init, NULL, NULL, &lscc_csi_rx_cfg_##inst,                 \
			      POST_KERNEL, CONFIG_VIDEO_INIT_PRIORITY, &lscc_csi_rx_driver_api);

DT_INST_FOREACH_STATUS_OKAY(LSCC_CSI_RX_DEVICE_DEFINE)
...
static int cmd_tvai_csi_rx_clear(const struct shell *sh, size_t argc, char **argv)
{
	const struct device *dev;
	const struct lscc_csi_rx_config *cfg;
...
static int cmd_tvai_csi_rx_set(const struct shell *sh, size_t argc, char **argv, uint32_t reg)
{
	const struct device *dev;
	const struct lscc_csi_rx_config *cfg;
	uint32_t cur_value;
...
static int cmd_tvai_csi_rx_settle(const struct shell *sh, size_t argc, char **argv)
{
	return cmd_tvai_csi_rx_set(sh, argc, argv, LSCC_CSI_RX_NOCIL_DSETTLE);
}
...
SHELL_CMD_REGISTER(tvai_csi_rx, &sub_tvai_csi_rx, "Lattice DPHY RX commands", NULL);
