...
#define GET_I2C_CFG(dev)						     \
	((const struct i2c_opencores_cfg *) dev->config)

#define GET_I2C_OPENCORES(dev)						     \
	((struct i2c_opencores *) dev->data)
...
static int opencores_i2c_configure(const struct device *dev,
			     uint32_t dev_config)
{
...
static bool i2c_wait_busy(const struct device *dev) {
  int waits = 0;
  struct i2c_opencores * context = GET_I2C_OPENCORES(dev);
  while(waits < 10000 && (opencores_read(dev, OC_I2C_SR) & (OC_I2C_TIP | OC_I2C_BUSY)) != 0) {
    k_busy_wait(10);
    waits++;
  }
  if(waits >= 10000) {
...
static bool i2c_write_byte(const struct device *dev, uint8_t byte) {
  ...
  context->next_cr = 0;
  if(!i2c_wait_tip(dev)) return false;
...
static int opencores_i2c_transfer(const struct device *dev,
			    struct i2c_msg *msgs,
...
{ 
	...
	ret = opencores_i2c_configure(dev, I2C_MODE_CONTROLLER | i2c_map_dt_bitrate(config->bitrate));
	if (ret != 0) {
...
I2C_DEVICE_DT_INST_DEFINE(n, i2c_opencores_init, NULL, &i2c_opencores_##n, &i2c_opencores_cfg_##n, PRE_KERNEL_1, \
			  CONFIG_I2C_INIT_PRIORITY, &opencores_i2c_api); 
